...
public class SpecialDaoImpl extends ApplicationDaoImpl<SpecialEntity> implements SpecialDao {
  ...
  public PaginatedListTo<SpecialEntity> findSpecials(SpecialSearchCriteriaTo criteria) {
    ...
    if (activePeriod != null) {
      WeeklyPeriod specialPeriod = special.getActivePeriod();

      query.where(Alias.$(specialPeriod.getEndingHour()).eq(activePeriod.getEndingHour()));
      query.where(Alias.$(specialPeriod.getStartingHour()).eq(activePeriod.getStartingHour()));

      DayOfWeek endingDay = activePeriod.getEndingDay();
      if (null != endingDay) {
        query.where(Alias.$(specialPeriod.getEndingDay()).eq(endingDay));
      }

      DayOfWeek startingDay = activePeriod.getStartingDay();
      if (startingDay != null) {
        query.where(Alias.$(specialPeriod.getStartingDay()).eq(startingDay));
      }
    }
